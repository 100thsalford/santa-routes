...
export default async (req, context) => {
  try {
    ...
    const [routeRows, dateRows, streetRows, settingsRows, volunteerRows, assignmentRows, roleRows] = await Promise.all([
      ...
      db.sql`SELECT id, route_id, to_char(event_date, 'YYYY-MM-DD') AS event_date, amount_collected, volunteer_capacity, what3words, notes, gathering_time, route_map_filename, route_map_content_type, route_track IS NOT NULL AS has_route_track, jsonb_array_length(route_track->'directions') AS route_track_turns, route_track->>'distanceKm' AS route_track_distance_km FROM route_dates ORDER BY event_date`,
    ...
    for (const d of dateRows) {
      ...
      (datesByRoute[d.route_id] = datesByRoute[d.route_id] || []).push({
        ...
        hasRouteTrack: !!d.has_route_track,
        routeTrackTurns: d.route_track_turns != null ? Number(d.route_track_turns) : null,
        routeTrackDistanceKm: d.route_track_distance_km != null ? Number(d.route_track_distance_km) : null,
